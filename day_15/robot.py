...
class Robot:
    ...
    def mark(self, delta: Coords, sign='.'):
        if self.mode != 'map':
            if (self.pos + delta) in self.map.keys() and not self.map[self.pos + delta] == 'O':
                self.map.pop(self.pos)
                self.counter -= 1
            else:
                self.map[self.pos + delta] = sign
                self.counter += 1
    ...
    def follow_wall(self, try_at, else_move) -> (int, int):
        ...
        if status == 0:
            ...
            return {1: 3, 2: 4, 3: 2, 4: 1}[try_at], try_at
    ...
    def scan(self, find_north_wall=True, initial_try_at=1, initial_else_move=4):
        if find_north_wall:
            ...
            while status == 1:
                status = self.move(1)
            if status == 2:
                print("the shortest path to oxygen is easy:", self.pos.y)
                return
        ...
        while not (start_scan_position == self.pos and try_at == initial_try_at):
            ...
            if self.mode == 'path' and self.map.get(self.pos, ' ') == 'O':
                break


if __name__ == '__main__':
    robot = Robot(StatefulIntcode(
