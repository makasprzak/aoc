...
from day_9.stateful_intcode import decode_opcode, StatefulIntcode

def intcode(inp: str):
    prog = inp.split(", ")
    prog = list(map(lambda x: int(x), prog))
    stateful_intcode = StatefulIntcode(prog, [])
    stateful_intcode.intcode_program()
    return stateful_intcode.program
